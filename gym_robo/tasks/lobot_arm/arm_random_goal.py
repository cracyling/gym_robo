...
class LobotArmRandomGoal:
    def __init__(self, robot, max_time_step: int = 500, accepted_dist_to_bounds=0.001,
                 ...
                 random_goal_seed=None, random_goal_file=None, normalise_reward=False, continuous_run=False, reward_noise_mu=None, reward_noise_sigma=None,
        ...
        self._max_time_step = max_time_step
        lobot_desc_share_path = get_package_share_directory('gym_robo_descriptions')
        arm_urdf_path = os.path.join(lobot_desc_share_path, 'robots/lobot_arm/lobot_arm.urdf')
        ...
        if isinstance(robot, LobotArmSim):  # Check if is gazebo
            ...
            robot.spawn_marker(target_x, target_y, target_z, 0.002, 1)
    ...
    def is_done(self, joint_angles: numpy.ndarray, has_contact: bool, observation_space: Box, time_step: int = -1) -> Tuple[bool, Dict]:
        failed, arm_state = self.__is_failed(joint_angles, has_contact, observation_space, time_step)
        ...
        current_coords = self.__get_coords(joint_angles)
        ...
        if arm_state == ArmState.Reached:
            ...
            if self.continuous_run:
                ...
                if isinstance(self.robot, LobotArmSim):  # Check if is gazebo
                    # Spawn the target marker if it is gazebo
                    self.robot.spawn_marker(self.target_coords[0], self.target_coords[1], self.target_coords[2], 0.002, 1)
    ...
    def __is_failed(self, joint_angles: numpy.ndarray, has_contact: bool, observation_space: Box, time_step: int = -1) -> Tuple[bool, ArmState]:
        ...
        upper_bound = observation_space.high[:3]  # First 3 values are the joint angles
        ...
        if min_dist_to_lower_bound < self.accepted_dist_to_bounds:
            joint_index = abs(joint_angles - lower_bound).argmin()
        ...
        if min_dist_to_upper_bound < self.accepted_dist_to_bounds:
            joint_index = abs(joint_angles - upper_bound).argmin()
            # print(f'Joint {joint_index} approach joint limits, '
            #       f'current joint value: {joint_states[joint_index]}, '
